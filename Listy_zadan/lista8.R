...
########################################
#ZAD 1. Na wbudowanych danych iris za pomocš rpart zbuduj drzewo klasyfikacyjne szacujšce przynależnoć do gatunku wykorzystujšc jako predyktory: 
#  a.	dane o Sepalach
#b.	wszystkie dane
#Porównaj procent właciwie zakwalifikowanych przypadków. Które drzewo lepiej klasyfikuje?
#Sprawd wpływ parametrów minsplit i cp  na głębokoć drzewa.
...
########################################
#ZAD 2. Teraz zbuduj drzewo z pomocš ctree, w dalszym cišgu wykorzystaj dane iris oraz wszystkie predyktory.
########################################
...
########################################
#ZAD 4. Na bazie ramki danych events stwórz następujšce zmienne dla wszystkich spraw z ramki cases i dodaj je do nowej ramki np. cases2
########################################
...
########################################
#ZAD 5. Podziel  zbiór cases2 na zbiór uczšcy i testowy w proporcji 60:40.
########################################
...
########################################
#ZAD 6. Sprawd działanie metod  (rpart, ctree) dla przypadków Y1,Y2 przy użyciu jednego predyktora: Age
########################################
table(uczace$y1)
...
#czułoć i specyficznoć
#SE (sensitivity, czułoć)  okrela zdolnoć klasyfikatora do wykrywania klasy pozytywnej 
#SE = TP / (TP + FN)
#SP (specificity, specyficznoć)  okrela zdolnoć klasyfikatora do wykrywania klasy negatywnej 
#SP = TN / (TN + FP)
#ACC (Total Accuracy)  całkowita sprawnoć klasyfikatora, okrela prawdopodobieństwo poprawnej klasyfikacji, czyli stosunek poprawnych klasyfikacji do wszystkich klasyfikacji
...
#########################################
#ZAD 7. Stwórz odpowiednie najlepsze (wg. macierzy klasyfikacji dla klasyfikacji i wg. błędu redniokwadratowego dla regresji) drzewo (odpowiednio do Y klasyfikacyjne, regresyjne) dla każdej ze zmiennych Y1, Y2, Y3 wybranš przez siebie metodš (rpart, ctree) wykorzystujšc tylko dane ze zbioru uczšcego. Zadbaj o to, by drzewo nie było przeuczone.
########################################


#budujemy wyłšcznie na zbiorze uczšcym!!!
###ad y1
summary(uczace)
names(uczace)
